...
| [liwi-resources-websocket-node-client](packages/liwi-resources-websocket-node-client) | <a href="https://npmjs.org/package/liwi-resources-websocket-node-client"><img src="https://img.shields.io/npm/v/liwi-resources-websocket-node-client.svg?style=flat-square"></a> | node websocket client implementation for liwi      |
...
- **Transports** — [websocket](packages/liwi-resources-websocket-client) for browsers and react-native, [websocket-node](packages/liwi-resources-websocket-node-client) for node hosts that need handshake auth headers, [direct](packages/liwi-resources-direct-client) for in-process calls, [void](packages/liwi-resources-void-client) for SSR. Payloads are encoded with [`extended-json`](packages/extended-json) so `Date` values survive.
